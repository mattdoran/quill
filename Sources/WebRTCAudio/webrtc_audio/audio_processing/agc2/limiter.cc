...
void ComputePerSampleSubframeFactors(
  ...
  const int subframe_size = CheckedDivExact(
      static_cast<int>(per_sample_scaling_factors.size()),
      static_cast<int>(num_subframes));
  ...
  if (is_attack) {
    InterpolateFirstSubframe(
        ...
        per_sample_scaling_factors.data(), subframe_size);
  }
...
void ScaleSamples(MonoView<float> per_sample_scaling_factors,
...
void Limiter::Process(DeinterleavedView<float> signal) {
  ...
  const std::array<float, kSubFramesInFrame> level_estimate =
      level_estimator_.ComputeLevel(DeinterleavedView<const float>(signal[0].data(), signal.samples_per_channel(), signal.num_channels()));
