 ...
 public:
  ...
  float Analyze(MonoView<const float> frame) override {
    ...
    const bool is_silence = features_extractor_.CheckSilenceComputeFeatures(
        /*samples=*/rtc::ArrayView<const float, rnn_vad::kFrameSize10ms24kHz>(frame.data(), rnn_vad::kFrameSize10ms24kHz),
        feature_vector);
...
float VoiceActivityDetectorWrapper::Analyze(
  ...
  return vad_->Analyze(MonoView<const float>(resampled_buffer_.data(), resampled_buffer_.size()));
}
