...
class NoiseSuppressor {
 public:
  ...
  // Returns the speech probability (0.0-1.0) from the most recent Analyze()
  // call, averaged across channels. Based on spectral features (LRT, flatness,
  // template difference), not simple energy.
  float GetSpeechProbability() const {
    if (channels_.empty()) return 0.f;
    float sum = 0.f;
    for (const auto& ch : channels_) {
      sum += ch->speech_probability_estimator.get_prior_probability();
    }
    return sum / static_cast<float>(channels_.size());
  }
