...
class ArrayView final : public impl::ArrayViewBase<T, Size> {
 public:
  ...
  std::reverse_iterator<T*> rbegin() const { return std::reverse_iterator<T*>(end()); }
  std::reverse_iterator<T*> rend() const { return std::reverse_iterator<T*>(begin()); }
  ...
  // span-style aliases used by upstream AGC2/rnn_vad code
  ArrayView<T> subspan(size_t offset, size_t count) const {
    return subview(offset, count);
  }
  ArrayView<T> subspan(size_t offset) const {
    return subview(offset);
  }
  template <size_t Offset, size_t Count>
  ArrayView<T, Count> subspan() const {
    return ArrayView<T, Count>(this->data() + Offset, Count);
  }
  template <size_t Count>
  ArrayView<T, Count> first() const {
